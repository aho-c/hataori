...
from ctypes.wintypes import WORD, DWORD, LONG, UINT, MAX_PATH
...
def __reg(text, pattern, ignorecase = False, multiline = False):
    match_flgs = 0
    if ignorecase:
        match_flgs = match_flgs | re.MULTILINE
    if multiline:
        match_flgs = match_flgs | re.IGNORECASE
...
def press_key_code(key_code):
    ...
    if SendInput(1, byref(input_structure), sizeof(INPUT)) != 0:
        return True
    return False

def release_key_code(key_code):
    if not type(key_code) is int: return False
...
def send_key_code(key_code):
    if not press_key_code(key_code): return False
    if not release_key_code(key_code): return False
    return True

def send_keys(string_key):
    lower_key = string_key.lower()
    keys = __reg(lower_key, r'^(([a-z0-9]*)(\+))*([a-z0-9]*)$')
    command_key = keys['groups'][1]
    key = keys['groups'][3]
    if key is None: return False
    if command_key == 'shift': press_key_code(VK_SHIFT)
    elif command_key == 'ctrl': press_key_code(VK_CONTROL)
    elif command_key == 'alt': press_key_code(VK_MENU)
    elif command_key == 'win': press_key_code(VK_LWIN)
    elif command_key is None: pass
    else: return False
    if key in key_function:
        press_key_code(key_function[key])
        release_key_code(key_function[key])
    else:
        return False
    if command_key == 'shift': release_key_code(VK_SHIFT)
    elif command_key == 'ctrl': release_key_code(VK_CONTROL)
    elif command_key == 'alt': release_key_code(VK_MENU)
    elif command_key == 'win': release_key_code(VK_LWIN)
    return True
...
def find_windows_by_class(class_name):
    ...
    def _enum_windows(hwnd, lParam):
        ...
        if not pattern.search(window_class_name) is None:
            hwnds.append(hwnd)
        return True
    WNDENUMPROC = WINFUNCTYPE(c_bool, POINTER(c_int), POINTER(c_int))
    EnumWindows(WNDENUMPROC(_enum_windows), 0)
