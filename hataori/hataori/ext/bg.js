...
axon.native_messaging.recv = (message) =>{
    try{
        if(axon.native_messaging.run){
            ...
            if((message_dict.gp === 'system') && (message_dict.fc === 'nothing')){
            ...
            }else if((message_dict.gp === 'browser') && (message_dict.fc === 'browser_exit')){
                axon.browser_exit();
                ...
                axon.browser.tabs.query({active: true, currentWindow: true}, function(tabs){
                    if(tabs.length === 0){
                        axon.native_messaging.send({ret: null, err: 'Unable to request tabs.'});
                    }else if((tabs[0].url.indexOf('chrome://') === 0) || (tabs[0].discarded)){
                        axon.native_messaging.send({ret: null, err: 'Unable to request tabs.' + tabs[0].url});
                    }else{
...
axon.window_action = (mode) =>{
    axon.browser.tabs.query({active: true, currentWindow: true}, function(tabs){
        try{
            if((mode === 'normal') || (mode === 'maximized') || (mode === 'fullscreen')){
